...
const LEARNING_MIN_SCHEDULE_MS = MINUTE_IN_DAYS * DAY_MS;
const RELEARNING_MIN_SCHEDULE_MS = RELEARNING_SCHEDULE_FLOOR_DAYS * DAY_MS;
const REVIEW_MIN_SCHEDULE_MS = REVIEW_SCHEDULE_FLOOR_DAYS * DAY_MS;
const LEARNING_MAX_SCHEDULE_MS = DAY_MS;
const RELEARNING_MAX_SCHEDULE_MS = RELEARNING_MAX_SCHEDULE_DAYS * DAY_MS;
const MAX_UPCOMING_HOURS = STABILITY_MAX * 24;
    ...
    try {
      ...
      if (typeof valueOf === 'function') {
        ...
        if (unboxed instanceof Date) {
          const unboxedDateMs = unboxed.getTime();
          if (Number.isFinite(unboxedDateMs)) {
            return toSafeIso(unboxedDateMs);
          }
        }
    ...
    try {
      ...
      if (typeof toString === 'function') {
        ...
        if (stringified instanceof Date) {
          const stringifiedDateMs = stringified.getTime();
          if (Number.isFinite(stringifiedDateMs)) {
            return toSafeIso(stringifiedDateMs);
          }
        }
