...
describe('hasScheduleRepairNeed', () => {
  ...
  it('accepts reviewing state aliases when schedule is otherwise healthy', () => {
    const reviewingAlias = {
      ...createNewCard('repair-reviewing-alias', 'test', NOW),
...
describe('resolveReviewClock', () => {
  ...
  it('accepts valueOf Date-backed clocks from runtime bridges', () => {
    const renderedClock = {
      valueOf() {
        return new Date('2026-02-23T12:00:00.000Z');
      },
    ...
    const runtimeClock = {
      valueOf() {
        return new Date('2026-02-23T12:00:10.000Z');
      },
    };

    expect(resolveReviewClock(renderedClock as unknown as string, runtimeClock as unknown as string)).toBe(
      '2026-02-23T12:00:10.000Z',
    );
  });
...
describe('resolveAddCardClock', () => {
  ...
  it('does not regress to epoch when runtime clock is pathologically stale', () => {
    const nowSpy = jest.spyOn(Date, 'now').mockReturnValue(Date.parse('2026-02-23T12:34:56.000Z'));
    const resolved = resolveAddCardClock('2026-02-23T12:34:30.000Z', '1970-01-01T00:00:00.000Z');
    nowSpy.mockRestore();
  ...
  it('accepts valueOf Date-backed add-card clocks from runtime bridges', () => {
    const nowSpy = jest.spyOn(Date, 'now').mockReturnValue(Date.parse('2026-02-23T12:00:00.000Z'));
    const renderedClock = {
      valueOf() {
        return new Date('2026-02-23T12:00:30.000Z');
      },
    };
    const runtimeClock = {
      valueOf() {
    ...
    const resolved = resolveAddCardClock(renderedClock as unknown as string, runtimeClock as unknown as string);
    nowSpy.mockRestore();
