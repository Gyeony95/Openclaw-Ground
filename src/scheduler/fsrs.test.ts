...
describe('fsrs scheduler', () => {
  ...
  it('anchors malformed timelines to wall clock when requested review time is exactly at future-skew boundary', () => {
    jest.useFakeTimers();
    try {
      jest.setSystemTime(new Date('2026-02-23T12:00:00.000Z'));
      const malformedTimelineCard = {
        ...createNewCard('malformed-timeline-future-boundary-anchor', 'safe', NOW),
        createdAt: 'not-a-date',
        updatedAt: 'not-a-date',
        dueAt: 'not-a-date',
        state: 'review' as const,
      ...
      const reviewed = reviewCard(malformedTimelineCard, 3, '2026-02-24T00:00:00.000Z').card;
  ...
  it('treats reviewed aliases as review state so mature cards keep review scheduling', () => {
    const reviewAliasCard = {
      ...createNewCard('reviewed-state-alias', 'state inference', NOW),
  ...
  it('keeps near-one-day on-time hard reviews from shrinking below a two-day bucket', () => {
    const runtimeCard = {
      ...createNewCard('daylike-hard-near-one-preserve-floor', 'interval floor', NOW),
      state: 'review' as const,
      updatedAt: NOW,
  ...
  it('keeps near-one-day on-time good reviews from shrinking below a two-day bucket', () => {
    const runtimeCard = {
      ...createNewCard('daylike-good-near-one-preserve-floor', 'interval floor', NOW),
      state: 'review' as const,
      updatedAt: NOW,
      dueAt: addDaysIso(NOW, 1.1),
      reps: 8,
  ...
  it('falls back to runtime wall clock when both fallback timeline and requested review clock are pathologically skewed', () => {
    jest.useFakeTimers();
    try {
      jest.setSystemTime(new Date('2026-02-23T14:30:00.000Z'));
      const staleCorrupted = {
        ...createNewCard('stale-corrupted-dual-skew', 'timeline', NOW),
        state: 'review' as const,
        updatedAt: '1900-01-01T00:00:00.000Z',
        dueAt: '1900-01-02T00:00:00.000Z',
        reps: 8,
        lapses: 2,
        stability: 4,
        difficulty: 5.5,
      ...
      const reviewed = reviewCard(staleCorrupted, 3, '2099-01-01T00:00:00.000Z');
  ...
  it('keeps rollback schedule anchors stability-aware for good review recalls', () => {
    jest.useFakeTimers();
    try {
      jest.setSystemTime(new Date('2026-02-23T14:30:00.000Z'));
      const futureCorrupted = {
        ...createNewCard('future-corrupted-good-anchor', 'timeline', NOW),
        state: 'review' as const,
        updatedAt: '2030-01-01T00:00:00.000Z',
      ...
      expect(reviewed.card.updatedAt).toBe('2026-02-23T14:30:00.000Z');
      expect(reviewed.card.state).toBe('review');
      expect(reviewed.scheduledDays).toBeGreaterThan(2);
    } finally {
      jest.useRealTimers();
    }
  });
  ...
  it('normalizes relearned aliases to relearning at runtime', () => {
    const relearnedAliasCard = {
      ...createNewCard('runtime-state-relearned', 'letter', NOW),
      state: ' re-learned ' as unknown as 'relearning',
      dueAt: NOW,
      updatedAt: NOW,
    };

    const reviewed = reviewCard(relearnedAliasCard, 3, NOW);
  ...
  it('trusts explicit valid review timestamps when runtime wall clock is non-finite', () => {
    const nowSpy = jest.spyOn(Date, 'now').mockReturnValue(Number.NaN);
    try {
      const corrupted = {
        ...createNewCard('sigma-nan-wallclock-review', 'letter', NOW),
        state: 'review' as const,
        updatedAt: '2030-01-01T00:00:00.000Z',
        dueAt: '2030-01-02T00:00:00.000Z',
        stability: 8,
  ...
  it('accepts near-integer runtime ratings during learning when values include tiny float drift', () => {
    const base = createNewCard('eta-near-int-learning', 'letter', NOW);
    const nearEasy = reviewCard(base, (4 - 4e-7) as unknown as Rating, NOW);
    const exactEasy = reviewCard(base, 4, NOW);

    expect(nearEasy.card.state).toBe(exactEasy.card.state);
  ...
  it('accepts near-integer runtime ratings during review when values include tiny float drift', () => {
    const base = createNewCard('eta-near-int-review', 'letter', NOW);
    const graduated = reviewCard(base, 4, NOW).card;
    const nearHard = reviewCard(graduated, (2 + 3e-7) as unknown as Rating, graduated.dueAt);
    const exactHard = reviewCard(graduated, 2, graduated.dueAt);
  ...
  it('caps outlier review dueAt repairs with malformed stability to the conservative fallback window', () => {
    const updatedAt = NOW;
    const conservative = {
      ...createNewCard('outlier-malformed-stability-valid', 'letter', NOW),
      state: 'review' as const,
      updatedAt,
      createdAt: NOW,
      dueAt: addDaysIso(updatedAt, 7),
    ...
    const outlierDue = {
      ...conservative,
      id: 'outlier-malformed-stability-invalid',
      dueAt: addDaysIso(updatedAt, 30),
    };
    const reviewAt = addDaysIso(updatedAt, 7);
  ...
  it('keeps very-early hard reviews from collapsing near-two-day schedules to one day', () => {
    const updatedAt = '2026-02-23T12:00:00.000Z';
    const card = {
      ...reviewCard(createNewCard('threshold-hard-over-early', 'definition', NOW), 4, NOW).card,
      state: 'review' as const,
      updatedAt,
      dueAt: addDaysIso(updatedAt, 1.76),
      stability: 1.76,
    ...
    const reviewed = reviewCard(card, 2, updatedAt);
  ...
  it('accepts primitive numeric review timestamps so runtime millisecond clocks schedule deterministically', () => {
    const card = {
      ...createNewCard('numeric-now-review', 'definition', NOW),
      state: 'review' as const,
      updatedAt: addDaysIso(NOW, -2),
      dueAt: NOW,
      reps: 5,
    ...
    const primitive = reviewCard(card, 3, NOW);
    const numericNow = reviewCard(card, 3, Date.parse(NOW) as unknown as string);

    expect(numericNow.scheduledDays).toBe(primitive.scheduledDays);
    expect(numericNow.card.updatedAt).toBe(primitive.card.updatedAt);
    expect(numericNow.card.dueAt).toBe(primitive.card.dueAt);
    expect(numericNow.card.reps).toBe(primitive.card.reps);
  });
  ...
  it('accepts primitive numeric preview timestamps so interval previews match canonical clocks', () => {
    const card = {
      ...createNewCard('numeric-now-preview', 'definition', NOW),
      state: 'review' as const,
      updatedAt: addDaysIso(NOW, -3),
      dueAt: NOW,
      reps: 6,
    ...
    const primitive = previewIntervals(card, NOW);
    const numericNow = previewIntervals(card, Date.parse(NOW) as unknown as string);

    expect(numericNow[1]).toBe(primitive[1]);
    expect(numericNow[2]).toBe(primitive[2]);
    expect(numericNow[3]).toBe(primitive[3]);
    expect(numericNow[4]).toBe(primitive[4]);
  });
  ...
  it('accepts primitive numeric creation timestamps so imported cards preserve numeric epoch anchors', () => {
    const created = createNewCard('numeric-create-now', 'definition', Date.parse(NOW) as unknown as string);
